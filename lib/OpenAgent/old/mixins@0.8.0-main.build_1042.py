...
# Auto-loaded OpenAgent runtime mixins for DoxTeam.
# Source: https://github.com/hairpin01/repo-DoxTeam-fork/tree/main/lib/OpenAgent
...
class _OpenAgentLifecycleMixin:
    ...
    async def on_load(self) -> None:
        ...
        defaults = {
            ...
            "skill_repo_url": "https://raw.githubusercontent.com/hairpin01/repo-DoxTeam-fork/main/OpenAgent/skills",
...
class OpenAgentProviderService:
    """Provider/model config helpers independent from DoxTeam runtime."""
...
class OpenAgentTodoService:
    """Pure TODO parsing/formatting logic, isolated from DoxTeam runtime."""
...
class OpenAgentToolDisplayService:
    """Tool status/rendering helpers that can be tested without DoxTeam runtime."""
...
class OpenAgentContextService:
    """Conversation-history helpers that do not depend on DoxTeam objects."""
...
class _OpenAgentPluginSkillMixin:
    ...
    def _refresh_live_config_schema(self) -> None:
        """Expose plugin-added ConfigValues to DoxTeam config UI."""
        with contextlib.suppress(Exception):
    ...
    def _core_tool_docs(self) -> dict[str, dict[str, str]]:
        return {
            ...
            "code.generate_doxteam_module": {"desc": "Generate an DoxTeam module file.", "args": "name", "body": "module code"},
            ...
            "code.read_docs": {"desc": "Read bundled/remote DoxTeam API documentation."},
    ...
    async def _fetch_repo_plugins(self) -> list[dict]:
        ...
        url = "https://api.github.com/repos/hairpin01/repo-DoxTeam-fork/contents/OpenAgent/plugins"
        try:
    ...
    async def _parse_plugin_meta(self, raw_url: str) -> dict:
        ...
        tools_m = re.findall(r'"((?:terminal|web|doxteam|message|file|dialog|chat|moderation|profile|contacts|creation|account|code|utility|skills|context|todo|thinking)\.[\w.]+)"', code)
    ...
    async def _install_plugin_from_repo(self, name: str) -> str:
        ...
        raw_url = f"https://raw.githubusercontent.com/hairpin01/repo-DoxTeam-fork/main/OpenAgent/plugins/{safe_name}.py"
        async with aiohttp.ClientSession() as session:
    ...
    def _repo_context_prompt(self) -> str:
        ...
        return "\n\nLocal DoxTeam workspace snapshot:\n" + text
    ...
    def _skill_repo_base_url(self) -> str:
        return str(
            self.config.get(
                "skill_repo_url",
                "https://raw.githubusercontent.com/hairpin01/repo-DoxTeam-fork/main/OpenAgent/skills",
            )
            or ""
        ).strip().rstrip("/")
...
class _OpenAgentRuntimeToolsMixin:
    ...
    def _system_prompt(self, user_prompt: str = "", flash_mode: bool = False) -> str:
        ...
        prompt += (
            ...
            "\n## What tools are\n"
            "Tools are OpenAgent operations that let you do work outside plain text: inspect the workspace, run terminal commands, use DoxTeam/Telegram actions, manage skills/todos/context, and call plugin features.\n"
            ...
            "Always call tools when you need external state, actions, files, Telegram/DoxTeam operations, or tool/plugin documentation.\n"
            "\n## Tool call format\n"
            ...
            "- When no tool is needed: reply in plain text with no ```tool_call``` blocks at all.\n"
             ...
             "2. doxteam.* tools: omit the userbot prefix (body='ping', not '.ping').\n"
...
class _OpenAgentTelegramMediaMixin:
    ...
    async def _fetch_doxteam_docs(self) -> str:
        timeout = aiohttp.ClientTimeout(total=int(self.config["timeout"]))
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(self.DoxTeam_DOCS_URL) as resp:
...
class _OpenAgentStatusMixin:
    ...
    def _requires_tool_confirmation(self, tool_name: str, attrs_raw: str = "", body: str = "") -> bool:
        ...
        critical_tools = {
            ...
            "doxteam.command", "doxteam.install", "doxteam.reload",
            ...
            "code.generate_file", "code.generate_doxteam_module", "code.attach_result",
        }
        critical_groups = {"terminal", "doxteam", "message", "file", "moderation", "profile", "contacts", "creation"}
        medium_groups = {
            "terminal", "doxteam", "message", "file", "moderation", "profile",
...
class _OpenAgentResponseMixin:
    ...
    def _sanitize_answer(self, text: str) -> str:
        patterns = [
            ...
            r"<(?:terminal|web|doxteam|message|file|dialog|chat|moderation|profile|contacts|creation|skills|context|utility|code)\.[^>]+>",
            r"</(?:terminal|web|doxteam|message|file|dialog|chat|moderation|profile|contacts|creation|skills|context|utility|code)\.[^>]+>",
        ]
...
class _OpenAgentToolRegistryMixin:
    ...
    async def _code_registry_tool(
        ...
        if tool_name == "code.generate_doxteam_module":
            filename = self._safe_generated_filename(name)
            if not filename.endswith(".py"):
                filename = self._safe_generated_filename(f"{Path(filename).stem}.py")
            if not content.strip():
                return "DoxTeam module code is required in tool body"
            self._last_generated_file = {"name": filename, "content": content}
            return f"DoxTeam module prepared: {filename} ({len(content)} chars)"
    ...
    async def _utility_registry_tool(self, tool_name: str, attrs_raw: str, body: str) -> str:
        ...
        if tool_name == "utility.error_file":
            return "Errors are reported through the DoxTeam kernel error handler"
    ...
    def _get_tool_map(self) -> dict[str, str]:
        ...
        core = {
            ...
            "code.generate_doxteam_module": "_code_registry_tool",
            "code.choose_filename": "_code_registry_tool",
            "code.attach_result": "_code_registry_tool",
            "code.read_docs": "_fetch_doxteam_docs",
    ...
    async def _dispatch_tool(
        ...
        *,
        ...
        try:
            ...
            # Parse XML attrs once; reused for mode/target/doxteam/skill branches below.
            attrs = self._parse_xml_attrs(attrs_raw)
            ...
            if method_name == "_run_doxteam_command" and not kwargs.get("command"):
                command_map = {
                    "doxteam.modules": "man",
                    "doxteam.config": "cfg",
                    "doxteam.install": "dlm",
