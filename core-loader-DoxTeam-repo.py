# name: core-loader-DoxTeam-repo
# requires: aiohttp
# author: @Hairpin00
# version: 1.0.2
# description: load new kernel
# banner_url: https://raw.githubusercontent.com/Heygen100/Module/main/DoxPNG.png
...
def register(kernel):
    ...
    lang = Strings(kernel, {
        'ru': {
            ...
            'read_error': 'Oшибкa чтeния иcтoчникa. Пoдpoбнocти зaпиcaны в лoг.',
            'empty_content': 'Пycтoe coдepжимoe',
            'no_kernel_class': 'Фaйл нe coдepжит клacca c имeнeм <b>Kernel</b>',
            'syntax_error': 'Cинтaкcичecкaя oшибкa в зaгpyжaeмoм фaйлe. Пoдpoбнocти зaпиcaны в лoг.',
            'save_failed': 'He yдaлocь coxpaнить ядpo. Пoдpoбнocти зaпиcaны в лoг.',
            'set_default_failed': 'He yдaлocь ycтaнoвить ядpo пo yмoлчaнию. Пoдpoбнocти зaпиcaны в лoг.',
            ...
            'download_file_error': 'Oшибкa зaгpyзки фaйлa. Пoдpoбнocти зaпиcaны в лoг.',
        ...
        'en': {
            ...
            'read_error': 'Error reading source. Details were written to the log.',
            'empty_content': 'Empty content',
            'no_kernel_class': 'The file does not contain a class named <b>Kernel</b>',
            'syntax_error': 'Syntax error in the uploaded file. Details were written to the log.',
            'save_failed': 'Failed to save kernel. Details were written to the log.',
            'set_default_failed': 'Failed to set default kernel. Details were written to the log.',
            'installed': 'Kernel <b>{name}</b> installed. Restarting with --core {name}...',
            'no_reply_file': 'Reply to a .py file or provide a source.',
            'download_file_error': 'Error downloading file. Details were written to the log.',
    ...
    async def core_load(event):
        ...
        if reply and reply.document:
            ...
            try:
                ...
                await answer(event, lang('download_file_error', error="details hidden"), as_html=True)
                return
            ...
            try:
                ...
                await answer(event, lang('read_error', error="details hidden"), as_html=True)
                return
...
async def _install_kernel(kernel, event, name, content, force, lang):
    ...
    try:
        ...
        await answer(event, lang('syntax_error', error="details hidden"), as_html=True)
        return
    ...
    try:
        ...
        await answer(event, lang('save_failed', error="details hidden"), as_html=True)
        return
    ...
    try:
        ...
        await answer(event, lang('set_default_failed', error="details hidden"), as_html=True)
