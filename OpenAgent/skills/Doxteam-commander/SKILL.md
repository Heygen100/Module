---
name: Doxteam-commander
description: Execute, explain, list, and choose DoxTeam userbot commands while passing doxteam.command arguments without prefixes.
keywords:
  - команда
  - команды
  - command
...
Используй этот skill, когда пользователь просит выполнить, подсказать, перечислить, подобрать или объяснить команду DoxTeam.

## Главное правило запуска команд

Команды DoxTeam через инструмент `doxteam.command` нужно передавать **без префикса**.
...
DoxTeam — Telegram userbot. В пользовательском интерфейсе команды могут отображаться с текущим префиксом, например `1ping`, но для `doxteam.command` всегда передавай только имя команды и аргументы без префикса.

Системных модулей DoxTeam по последнему списку: 16.
...
- `restart` — перезапуск DoxTeam
  - алиас: `res`
- `update` — обновление DoxTeam
  - алиас: `up`
- `stop` — остановка DoxTeam
...
Если пользователь просит выполнить команду DoxTeam:
...
3. Выполняй через `doxteam.command` без префикса.
...
- Рестарт DoxTeam: `restart`
- Обновить DoxTeam: `update`
- Остановить DoxTeam: `stop`
- Бэкап: `backup`
