---
name: DoxTeam-debugger-fixer
description: Run, interpret, and fix DoxTeam debugger warnings and errors while preserving module behavior and build history.
keywords:
  - debugger
  - debug
  - дeбaггep
...
# DoxTeam Debugger Fixer

Use this skill when the user asks to run, read, explain, or fix output from the DoxTeam debugger, especially:

- `python -m debugger.cli <module.py>`
- `doxteam-debugger <module.py>`
- debugger warnings like `DoxTeam001`, `DoxTeam027`, etc.
- requests like "пoчини вapны", "debugger pyгaeтcя", "иcпpaвь мoдyль пo дeбaггepy".

## Main goal

Turn debugger output into safe DoxTeam-compatible fixes without breaking module behavior. Treat debugger **errors** as blockers. Treat warnings as signals: fix real runtime/safety issues, but do not blindly rewrite valid DoxTeam patterns.
...
DoxTeam userbot handlers should usually be async:
...
Do not add useless `await asyncio.sleep(0)` unless there is no better fix and the function must remain async because DoxTeam expects it.
...
- debugger rule is too strict for a valid DoxTeam class-style pattern;
...
- Do not replace DoxTeam APIs with Hikka/Heroku APIs.
