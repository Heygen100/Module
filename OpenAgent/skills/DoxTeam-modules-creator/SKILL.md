---
name: DoxTeam-modules-creator
description: Create and update debug DoxTeam userbot modules in app-debug using the DoxTeam module API, class-style modules, and inline form/callback docs.
keywords:
  - мoдyль
  - мoдyли
  - module
...
# DoxTeam Modules Creator

Use this skill when the user asks to create, edit, update, refactor, or debug an DoxTeam module.

## Core identity and context

- DoxTeam is a **Telegram userbot**: it runs on a Telegram user account via Telethon/Telethon-DoxTeam.
- Do **not** assume DoxTeam is a regular Telegram Bot API bot running only on a bot account.
...
- Inline buttons/forms still involve the inline bot layer, but the module itself is for the DoxTeam userbot runtime.
- Never confuse DoxTeam modules with Hikka modules. DoxTeam uses `core.lib.loader.module_base` and `ModuleBase`; Hikka-style `from .. import utils, loader` and `loader.Module` are not DoxTeam module style.
...
- Use **only the DoxTeam API** for module structure, registration, commands, inline forms, callbacks, config, database, cache, lifecycle, logging, and helpers.
- Do not invent cross-framework helpers or import APIs from Hikka/Heroku userbot frameworks.
- External Python libraries are allowed only when they are genuinely needed for the module's domain and DoxTeam does not provide that functionality.
...
- When using an external library:
  ...
  - do not replace DoxTeam SDK functionality with third-party libraries.
...
Prefer class-style DoxTeam modules based on `ModuleBase`.
...
- Replace `{module_name}` with the actual DoxTeam module `name` value.
...
- Keep DoxTeam SDK imports under a `# SDK` comment.
- Prefer this DoxTeam SDK style for consistency:
...
## DoxTeam vs Hikka module style

Correct DoxTeam style:
...
Never convert DoxTeam modules into Hikka modules. Never use `loader.Module` for DoxTeam class-style modules; use `loader.ModuleBase`.
...
5. Confirm all imports are needed and valid for DoxTeam docs.
...
9. Confirm the module uses DoxTeam API only, except justified external domain libraries.
...
13. Run the DoxTeam debugger against the generated file:

```bash
python -m debugger.cli app-debug/{module-name}/modules-debug-v{version}.py
```

If the `doxteam-debugger` console script is installed, this is also acceptable:

```bash
doxteam-debugger app-debug/{module-name}/modules-debug-v{version}.py
```

Debugger rules:

- Treat debugger errors as blockers.
- Fix warnings when they indicate real DoxTeam runtime issues.
- If a warning is intentionally safe for a class-style DoxTeam API pattern, mention it explicitly in the final response.

## Update workflow

When the user asks to update an existing DoxTeam module:
...
When the user asks to create a new DoxTeam module:
...
7. Verify syntax.
8. Run the DoxTeam debugger from `debugger/` via `python -m debugger.cli app-debug/{module-name}/modules-debug-v{version}.py`.

## Do not do

- Do not treat DoxTeam as a regular Telegram Bot API framework.
- Do not confuse DoxTeam modules with Hikka modules.
- Do not use Hikka-style `from .. import utils, loader`.
- Do not inherit from `loader.Module`; use `loader.ModuleBase`.
- Do not use non-DoxTeam APIs for module registration/config/database/inline/lifecycle when DoxTeam API exists.
...
- Do not invent APIs when local docs provide the correct DoxTeam API.
