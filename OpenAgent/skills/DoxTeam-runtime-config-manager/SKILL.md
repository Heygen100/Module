---
name: DoxTeam-runtime-config-manager
description: Safely inspect and manage DoxTeam runtime configuration, prefixes, aliases, trusted users, access rules, and backups with secret masking and confirmations.
keywords:
  - cfg
  - fcfg
  - setprefix
...
# DoxTeam Runtime Config Manager

Use this skill when the user asks to inspect or change DoxTeam runtime settings through commands such as `cfg`, `fcfg`, `setprefix`, `aliases`, `trusted`, `access`, `backup`, or restore operations.

## Main goal

Safely manage DoxTeam runtime configuration without leaking secrets or accidentally weakening access controls. Prefer read-only inspection first, backup before mutation, verification after mutation, and a concise record of old/new non-secret state.

## Core rules

- DoxTeam is a Telegram userbot; prefix, trusted-user, and access changes affect user-account automation.
...
- Use `Doxteam-commander` for actual command execution or command explanation when needed.
- Pass `doxteam.command` arguments without command prefixes because `Doxteam-commander` expects raw command text.
...
6. Execute through `Doxteam-commander` when command execution is needed.
