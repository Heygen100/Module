---
name: DoxTeam-module-auditor
description: Statically audit DoxTeam modules before release for safety, metadata, dependencies, commands, inline callbacks, and release readiness without editing code.
keywords:
  - audit
  - review
  - static
...
# DoxTeam Module Auditor

Use this skill when the user asks to audit, review, inspect, or assess an DoxTeam module before release without changing module code.

## Main goal

Statically review DoxTeam userbot modules for safety, DoxTeam compatibility, metadata completeness, dependency clarity, command and inline callback readiness, and release blockers.
...
- Do not assume Hikka conventions are acceptable in DoxTeam modules.
- Recommend syntax check and DoxTeam debugger validation, but keep this workflow static unless the user separately asks to run tools.
...
4. Review DoxTeam API style and flag Hikka-style patterns.
...
### DoxTeam style and Hikka mismatch

- Expected DoxTeam style includes imports from `core.lib.loader.module_base`, `loader.ModuleBase`, and `@loader.command` / `@loader.callback` patterns.
- Flag Hikka-style imports such as `from .. import utils, loader` and base classes such as `loader.Module`.
- Flag Hikka-only assumptions, helpers, or `strings` name-only conventions as insufficient for DoxTeam release readiness.
...
Use `READY` only when no safety or compatibility blockers are found. Use `READY WITH WARNINGS` when release is plausible but improvements or manual confirmations remain. Use `BLOCKED` for secrets, destructive behavior, suspicious execution/exfiltration, syntax-breaking structure, or clear non-DoxTeam/Hikka implementation patterns.
