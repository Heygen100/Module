---
name: DoxTeam-release-modules
description: Validate, publish, index, commit, and push DoxTeam modules from debug builds into the DoxTeam module repository.
keywords:
  - release
  - publish
  - peлиз
...
# DoxTeam Release Modules

Use this skill when the user asks to release, publish, upload, ship, or update an DoxTeam module in the public DoxTeam module repository.

This skill is for moving an already-created DoxTeam module from the DoxTeam-fork/debug workspace into the release repository:
...
- DoxTeam modules are **DoxTeam userbot modules**, not Hikka modules and not regular Telegram Bot API bots.
- Release only modules that use the DoxTeam API correctly.
...
```text
/home/alina/test/DoxTeam/DoxTeam-fork/app-debug/{module-name}/modules-debug-v{version}.py
```

Release destination:

```text
/home/alina/test/DoxTeam/repo-DoxTeam-fork/{name-modules}-DoxTeam-repo.py
```
...
```text
Weather-DoxTeam-repo.py
speedtest-DoxTeam-repo.py
menu-buttons-DoxTeam-repo.py
```
...
```text
/home/alina/test/DoxTeam/repo-DoxTeam-fork/modules.ini
```
...
```text
speedtest-DoxTeam-repo.py   -> modules.ini entry: speedtest-DoxTeam-repo
Weather-DoxTeam-repo.py     -> modules.ini entry: Weather-DoxTeam-repo
```
...
3. Run syntax check from the DoxTeam-fork repository root:

```bash
python -m py_compile app-debug/{module-name}/modules-debug-v{version}.py
```

4. Run DoxTeam debugger from the DoxTeam-fork repository root:
...
6. Fix warnings when they indicate real DoxTeam runtime issues.
...
Use this workflow when the module is not already present in `/home/alina/test/DoxTeam/repo-DoxTeam-fork/`.

1. Validate source module syntax.
2. Validate source module with DoxTeam debugger.
3. Determine release file name:

```text
{name-modules}-DoxTeam-repo.py
```
...
Use this workflow when `/home/alina/test/DoxTeam/repo-DoxTeam-fork/{name-modules}-DoxTeam-repo.py` already exists.

1. Validate the new source module syntax.
2. Validate the new source module with DoxTeam debugger.
...
6. Verify `modules.ini` contains the exact entry `{name-modules}-DoxTeam-repo`.
...
```bash
git add {name-modules}-DoxTeam-repo.py
...
1. Run `git status --short` in `/home/alina/test/DoxTeam/repo-DoxTeam-fork`.
...
- Do not skip DoxTeam debugger validation.
- Do not release Hikka modules as DoxTeam modules.
- Do not let `modules.ini` use a name different from the release file stem.
- Do not add `.py` to `modules.ini`.
- Do not commit unrelated files.
- Do not push from the DoxTeam-fork repo when the release target is `repo-DoxTeam-fork`.
