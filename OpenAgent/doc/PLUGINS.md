...
OpenAgent uses a **dynamic plugin system** similar to how DoxTeam loads modules:
...
```
OpenAgent-DoxTeam-repo.py          # module (loader, core tools, dispatch)
OpenAgent/
...
await self.agent._run_doxteam_command(cmd, ev)  # DoxTeam command
...
| `doxteam.py` | `doxteam` | `doxteam.command`, `.config`, `.modules`, `.install`, `.reload` | DoxTeam kernel commands |
