...
# Ported from Hikari Teledocs for DoxTeam API only.
#
#             █ █ ▀ █▄▀ ▄▀█ █▀█ ▀
#             █▀█ █ █ █ █▀█ █▀▄ █
#              © Copyright 2022
...
class Teledocs(ModuleBase):
    ...
    def _inline_query_from_event(event: events.InlineQuery.Event) -> str:
        """Extract user query from DoxTeam inline event.

        DoxTeam routes inline handlers by the first word of ``event.text`` and passes
